export declare const nxVersion: any;
export declare const reactVersion = "18.2.0";
export declare const reactDomVersion = "18.2.0";
export declare const reactIsVersion = "18.2.0";
export declare const swcLoaderVersion = "0.1.15";
export declare const swcCoreVersion = "^1.2.173";
export declare const typesReactVersion = "18.0.25";
export declare const typesReactDomVersion = "18.0.9";
export declare const typesReactIsVersion = "17.0.3";
export declare const typesNodeVersion = "18.11.9";
export declare const babelPresetReactVersion = "^7.14.5";
export declare const styledComponentsVersion = "5.3.6";
export declare const typesStyledComponentsVersion = "5.1.26";
export declare const emotionStyledVersion = "11.10.5";
export declare const emotionReactVersion = "11.10.5";
export declare const emotionBabelPlugin = "11.10.5";
export declare const styledJsxVersion = "5.1.0";
export declare const reactRouterDomVersion = "6.4.3";
export declare const typesReactRouterDomVersion = "5.3.3";
export declare const testingLibraryReactVersion = "13.4.0";
export declare const reduxjsToolkitVersion = "1.9.0";
export declare const reactReduxVersion = "8.0.5";
export declare const reactTestRendererVersion = "18.2.0";
export declare const eslintPluginImportVersion = "2.26.0";
export declare const eslintPluginJsxA11yVersion = "6.6.1";
export declare const eslintPluginReactVersion = "7.31.11";
export declare const eslintPluginReactHooksVersion = "4.6.0";
export declare const babelPluginStyledComponentsVersion = "1.10.7";
export declare const tsLibVersion = "^2.3.0";
export declare const postcssVersion = "8.4.19";
export declare const tailwindcssVersion = "3.2.4";
export declare const autoprefixerVersion = "10.4.13";
export declare const expressVersion = "~4.18.2";
export declare const typesExpressVersion = "4.17.14";
export declare const isbotVersion = "^3.6.5";
export declare const corsVersion = "~2.8.5";
export declare const typesCorsVersion = "~2.8.12";
export declare const moduleFederationNodeVersion = "~0.9.6";
export declare const lessVersion = "3.12.2";
export declare const sassVersion = "^1.55.0";
export declare const stylusVersion = "^0.55.0";
