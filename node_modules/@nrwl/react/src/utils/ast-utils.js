...
function findMainRenderStatement(source) {
    ...
    const calls = (0, typescript_1.findNodes)(source, ts.SyntaxKind.CallExpression);
    ...
    const imports = (0, typescript_1.findNodes)(source, ts.SyntaxKind.ImportDeclaration);
...
function findDefaultExportDeclaration(source) {
    ...
    if (identifier) {
        const variables = (0, typescript_1.findNodes)(source, ts.SyntaxKind.VariableDeclaration);
        const fns = (0, typescript_1.findNodes)(source, ts.SyntaxKind.FunctionDeclaration);
        const cls = (0, typescript_1.findNodes)(source, ts.SyntaxKind.ClassDeclaration);
...
function findExportDeclarationsForJsx(source) {
    const variables = (0, typescript_1.findNodes)(source, ts.SyntaxKind.VariableDeclaration);
    const variableStatements = (0, typescript_1.findNodes)(source, ts.SyntaxKind.VariableStatement);
    const fns = (0, typescript_1.findNodes)(source, ts.SyntaxKind.FunctionDeclaration);
    const cls = (0, typescript_1.findNodes)(source, ts.SyntaxKind.ClassDeclaration);
    const exportDeclarations = (0, typescript_1.findNodes)(source, ts.SyntaxKind.ExportDeclaration);
    let componentNamesNodes = [];
    exportDeclarations.forEach((node) => {
        componentNamesNodes = [
            ...componentNamesNodes,
            ...(0, typescript_1.findNodes)(node, ts.SyntaxKind.ExportSpecifier),
        ];
    });
    ...
    const exported = nodesContainingJSX.filter((x) => {
        var _a, _b, _c;
        foundExport = (0, typescript_1.findNodes)(x, ts.SyntaxKind.ExportKeyword);
        if (x.kind === ts.SyntaxKind.VariableStatement) {
            const nameNode = (_a = (0, typescript_1.findNodes)(x, ts.SyntaxKind.VariableDeclaration)) === null || _a === void 0 ? void 0 : _a[0];
            return (((_b = nameNode === null || nameNode === void 0 ? void 0 : nameNode.name) === null || _b === void 0 ? void 0 : _b.kind) === ts.SyntaxKind.Identifier ||
            ...
            return ((x.name.kind === ts.SyntaxKind.Identifier && (foundExport === null || foundExport === void 0 ? void 0 : foundExport.length)) ||
...
function findDefaultExportIdentifier(source) {
    const exports = (0, typescript_1.findNodes)(source, ts.SyntaxKind.ExportAssignment);
...
function getComponentNode(sourceFile) {
    ...
    if (!(defaultExport &&
        ((0, typescript_1.findNodes)(defaultExport, ts.SyntaxKind.JsxElement).length > 0 ||
            (0, typescript_1.findNodes)(defaultExport, ts.SyntaxKind.JsxSelfClosingElement).length >
                0))) {
        return null;
    }
...
function getComponentPropsInterface(sourceFile, cmpDeclaration) {
    ...
    ts.isClassDeclaration(cmpDeclaration) &&
        ...
        if (heritageClause) {
            ...
            if (propsTypeExpression && propsTypeExpression.typeArguments) {
                propsTypeName = (_b = propsTypeExpression.typeArguments[0].typeName) === null || _b === void 0 ? void 0 : _b.getText();
            }
        }
    }
    else {
    ...
    if (propsTypeName) {
        return (0, typescript_1.findNodes)(sourceFile, ts.SyntaxKind.InterfaceDeclaration).find((x) => {
