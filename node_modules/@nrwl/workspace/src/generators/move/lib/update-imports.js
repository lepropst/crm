...
function updateImports(tree, schema, project) {
    ...
    if (tsConfig) {
        ...
        const updatedPath = path.map((x) => (0, devkit_1.joinPathFragments)(projectRoot.to, (0, path_2.relative)(projectRoot.from, x)));
        if (schema.updateImportPath) {
...
function updateImportDeclarations(sourceFile, from, to) {
    const importDecls = (0, typescript_2.findNodes)(sourceFile, ts.SyntaxKind.ImportDeclaration);
    const changes = [];
...
function updateDynamicImports(sourceFile, from, to) {
    const expressions = (0, typescript_2.findNodes)(sourceFile, ts.SyntaxKind.CallExpression);
    const changes = [];
